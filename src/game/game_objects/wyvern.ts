import { ArcadeBody, Wyvern, WyvernDriver } from "./wyvernDriver";
...
export function createWyvern(
    ...
    const driver = new WyvernDriver(world, eid);
    ...
    driver.setTimeRate(sizeConfig[size].rate);

    // Connect the controller to the animation driver and the physics body.
    driver.comms
        .when('useSkill', (callback) => callback(sprite, effectsGroup, Heading[eid]));
    
    driver.startTick(sprite.scene);

    ecs.addComponent(world, eid, Initialize);
    return { eid, sprite: sprite, skillset: driver, size };
}
